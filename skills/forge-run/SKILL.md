...
3. **Implement.** Dispatch an implementer subagent whose prompt contains the FULL issue text + PRD's Allowed APIs & anti-patterns + CONTEXT.md glossary + worktree path — never "read the plan file yourself". The implementer follows `forge-tdd`; on bugs, `forge-rootcause`. It reports one of: `DONE` / `DONE_WITH_CONCERNS` / `NEEDS_CONTEXT` / `BLOCKED` — with evidence (commands run, output).
